(function() {
    ...
    var save = function*(record, entitySchema, ceramic, db) {
        ...
        if (!errors) {
            if (db.getRowId(record, entitySchema) && (entitySchema.concurrency === 'optimistic' || !entitySchema.concurrency)) {
                var _item = yield* db.findOne(entitySchema.collection, getIdQuery(record, entitySchema));
            ...
            var result;
            if (!db.getRowId(record, entitySchema)) {
                ...
                db.setRowId(record, result.insertedId, entitySchema);
            } else {
        ...
        if (link.key) {
            ...
            if (typeof link.field === 'string') {
                switch (otherEntitySchema.schema.properties[link.field].type) {
                    case 'string':
                        var params = {};
                        params["" + link.field] = db.getRowId(record, entitySchema);
