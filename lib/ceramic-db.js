(function() {
    ...
    var find = function*(entitySchema, query, p1, p2, p3) {
        ...
        if (items.length) {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                results.push(yield* ceramic.constructModel(item, entitySchema, { systemFields: ["__updateTimestamp"] }));
            }
            return results;
        } else {
            return [];
    ...
    var findOne = function*(entitySchema, query, p1, p2, p3) {
        ...
        if (result) {
            return yield* ceramic.constructModel(result, entitySchema, { systemFields: ["__updateTimestamp"] });
        }
    };
