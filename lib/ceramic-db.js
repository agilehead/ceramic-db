(function() {
    ...
    var find = function*(typeDefinition, query, p1, p2, p3) {
        ...
        if (items.length) {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                results.push(yield* ceramic.constructModel(item, typeDefinition, { systemFields: ["__updateTimestamp", "__shard"] }));
            }
            return results;
        } else {
            return [];
    ...
    var findOne = function*(typeDefinition, query, p1, p2, p3) {
        ...
        if (result) {
            return yield* ceramic.constructModel(result, typeDefinition, { systemFields: ["__updateTimestamp", "__shard"] });
        }
    };
