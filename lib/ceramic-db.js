(function() {
    ...
    var find = function*(typeDefinition, query, options, context) {
        ...
        var db = context.db, typesManager = context.typesManager;
        ...
        if (items.length) {
            ...
            for (var _i = 0; _i < items.length; _i++) {
                var item = items[_i];
                results.push(yield* typesManager.constructModel(item, typeDefinition, { systemFields: ["__updateTimestamp", "__shard"] }));
            }
            return results;
        } else {
            return [];
    ...
    var findOne = function*(typeDefinition, query) {
        ...
        if (result) {
            return yield* typesManager.constructModel(result, typeDefinition, { systemFields: ["__updateTimestamp", "__shard"] });
        }
    };
    ...
    var save = function*(record, typeDefinition, context, options) {
        ...
        var errors = yield* typesManager.validate(record, typeDefinition);
        ...
        if (!errors.length) {
            ...
            if (!db.getRowId(record)) {
                ...
                if (!options || !options.modifyCurrent) {
                    result = yield* typesManager.constructModel(insertedObject, typeDefinition, { systemFields: ["__updateTimestamp", "__shard"] });
                } else {
    ...
    var link = function*(record, typeDef, name, context) {
        ...
        var otherTypeDef = yield* typesManager.getTypeDefinition(link.type);
        if (link.key) {
